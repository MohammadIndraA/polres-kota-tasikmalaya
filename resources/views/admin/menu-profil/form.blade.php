@extends('layouts.app', ['title' => 'Form Menu Profil'])
@section('content')
                ...
                <div class="w-full relative p-4">
                    ...
                    <div class="mb-6">
                        ...
                        <div id="drop-area-menu-profil"
                            ...
                            <input type="file" name="image" id="image-menu-profil" class="hidden" accept="image/*">
                        </div>

                        <div class="mt-4 text-center">
                            <img id="image-preview-menu-profil"
                                src="{{ isset($menuProfil) && $menuProfil->image ? asset('storage/' . $menuProfil->image) : '' }}"
                                alt="Preview Gambar" class="mx-auto rounded-md"
                                style="max-width: 200px; max-height: 200px; {{ isset($menuProfil) && $menuProfil->image ? 'display: block;' : 'display: none;' }}">
                        </div>
                    ...
                    <div class="mb-6">
                        <label for="image-1" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Gambar / File / PDF
                        </label>
                        ...
                        <div id="drop-area-1"
                            ...
                            <input type="file" name="dokumen[]" id="image-1" class="hidden"
                                accept="image/*,application/pdf" multiple>
                        </div>

                        {{-- Preview file baru --}}
                        <div id="image-preview-1" class="mt-4 grid grid-cols-2 md:grid-cols-3 gap-5"></div>

                        {{-- Preview file lama (edit mode) --}}
                        @if (!empty($menuProfil->dokumen))
                            <div id="existing-preview" class="mt-4 grid grid-cols-2 md:grid-cols-3 gap-4">
                                @foreach ($menuProfil->dokumen as $path)
            ...
            <div
                ...
                <div class="w-full relative mb-4">
                    <div class="flex-auto p-0 sm:p-4">
                        ...
                        {{-- nama --}}
                        <x-input-v2 name="name" label="Nama" :value="old('name', $menuProfil->name ?? '')" placeholder="Nama Menu Profil"
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            initImageUploader('drop-area-menu-profil', 'image-menu-profil', 'image-preview-menu-profil');
            initFileMultipleUploader('drop-area-1', 'image-1', 'image-preview-1');
